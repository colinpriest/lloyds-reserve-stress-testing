...
This toolkit provides four complementary pipelines:
...
4. **Stress Testing Pipeline** - Builds a validated scenario library using EVT + LLM generation, then queries portfolio-specific stress scenarios by return period

Additionally, the project includes **novelty analyses** for the IME paper submission, covering sampling sensitivity, mix trends, tail stability, size-scaling validation, and capital distortion.
...
- **Stress test scenarios**: Portfolio-specific stress scenarios with severity calibrated by GPD and narratives generated by LLMs
...
├── simulation-workflow.md                  # Stress test architecture documentation
...
│
├── IME/                                    # IME paper outputs
│   ├── figure{1-6}_*.{pdf,png}             # Paper figures
│   ├── table{1-5}_*.tex                    # LaTeX tables
...
│       └── plots/                          # Diagnostic visualizations (incl. EVT)
│
├── scripts/
...
│       ├── coherence_validation.py         # Coherence validation
...
│       ├── diagnostic_visualizer.py        # Diagnostic visualization
│       ├── library_diagnostics.py          # Scenario library diagnostics
...
│       └── novelty/                        # Novelty analyses for IME paper
│           ├── run_all.py                  # Run all novelty analyses
│           ├── verdicts.py                 # Verdict aggregation
│           ├── novelty_0_sampling_sensitivity.py
│           ├── novelty_1_mix_trend.py
│           ├── novelty_2_tail_stability.py
│           ├── novelty_3_size_scaling_validation.py
│           ├── novelty_4_capital_distortion.py
...
The stress testing pipeline builds a validated library of synthetic stress scenarios from historical data, then retrieves and customises scenarios for specific portfolios. See [simulation-workflow.md](simulation-workflow.md) for full architectural details and [scripts/stress_test/README.md](scripts/stress_test/README.md) for module reference.

### Phase 1: Build Scenario Library (~2 hours)

```bash
# Full build (includes LLM generation)
python scripts/stress_test/pipeline.py build \
    --corpus results/combined/unified_corpus.json \
    --output results/stress_test/

# Dual-mode build (strict RAG-only + estimated LLM-fallback)
python scripts/stress_test/dual_mode_pipeline.py \
    --corpus results/combined/unified_corpus.json \
    --output results/stress_test/
```

Steps: Data preparation (severity ratios, complexity scores) → Joint embedding (3D latent space) → GPD threshold selection (MRL + parameter stability + A-D test) → Stratified LLM generation → Semantic coverage validation → Importance sampling → Coherence validation.

### Phase 2: Portfolio Query (<5 seconds)
...
Converts return period to severity threshold via GPD, filters scenarios by LOB composition and complexity, fine-tunes for portfolio, and generates chain-of-thought explanations.

### Novelty Analyses (IME Paper)

Five quantitative analyses validating the methodology, run from `scripts/stress_test/novelty/`:

```bash
# Run all analyses and produce verdicts
python scripts/stress_test/novelty/run_all.py
```

| Analysis | Purpose |
|----------|---------|
| `novelty_0` | Sampling sensitivity — bootstrap robustness of GPD parameters |
| `novelty_1` | Mix trend — temporal stability of LOB composition |
| `novelty_2` | Tail stability — GPD fit consistency across time windows |
| `novelty_3` | Size-scaling validation — severity elasticity by syndicate size |
| `novelty_4` | Capital distortion — portfolio-specific capital impact |

Outputs: figures in `novelty/fig/`, results in `novelty/results/`, LaTeX tables/figures in `IME/`.
...
| [simulation-workflow.md](simulation-workflow.md) | Stress test architecture (Phase 1 + Phase 2) |
| [file_and_folder_structure.md](file_and_folder_structure.md) | Complete directory tree |
| [scripts/stress_test/README.md](scripts/stress_test/README.md) | Stress test module reference |
...
| [query_documentation.md](query_documentation.md) | Portfolio query documentation |
| [query_upgrade_documentation.md](query_upgrade_documentation.md) | Portfolio query upgrade documentation |
